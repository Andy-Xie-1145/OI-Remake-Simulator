...
std::string BuildEndingSummary() {
    if (playerStats.isIOIgold) return "你成功拿到了 IOI 金牌，最终还是站在了世界 OI 之巅。";
    if (playerStats.isNationalTeam) return "你成为了中国国家队选手，代表中国参加了 IOI。";
    if (playerStats.isTrainingTeam) return "你作为国家集训队选手，已经具备了保送资格。";
    if (playerStats.isProvincialTeam) return "作为省队选手，你在 OI 的道路上已经取得了不错的成绩。";
...
std::string BuildRequirementText(const SubProblem& sp, int problemIdx, int subProblemIdx) {
    ...
    if (sp.dp > 0) parts.push_back("动态规划:" + visibleValue(sp.dp));
    if (sp.ds > 0) parts.push_back("数据结构:" + visibleValue(sp.ds));
    if (sp.str > 0) parts.push_back("字符串:" + visibleValue(sp.str));
    if (sp.graph > 0) parts.push_back("图论:" + visibleValue(sp.graph));
    if (sp.comb > 0) parts.push_back("组合计数:" + visibleValue(sp.comb));
    if (sp.thinking > 0) parts.push_back("思维:" + std::to_string(sp.thinking));
    if (sp.coding > 0) parts.push_back("代码:" + std::to_string(sp.coding));
...
int GuiApp::TotalAllocated() const {
    int total = 0;
    for (int value : talents_) total += value;
    return total;
}
...
void GuiApp::SelectTrainingOption(size_t index) {
    if (training_.currentEventKey.empty()) return;
    auto eventIt = TRAINING_EVENTS.find(training_.currentEventKey);
    if (eventIt == TRAINING_EVENTS.end() || index >= training_.options.size()) return;
    ...
    if (event.isShop) {
        ...
        if (playerStats.determination < selected.cost) {
            logEvent("决心不足！需要" + std::to_string(selected.cost) + "点决心，当前只有" +
                std::to_string(playerStats.determination) + "点。", "event");
            StartTrainingEvent();
            return;
        }
        ...
        if (incIt != SHOP_PRICE_INCREMENTS.end()) {
            ...
            if (priceIt != incIt->second.end()) {
                ...
                logEvent("下次购买" + selected.text + "需要" +
                    std::to_string(currentShopPrices[selected.text]) + "点决心", "event");
            }
        }
...
void GuiApp::ContinueTrainingPhase() {
    if (!training_.awaitingContinue) return;
...
void GuiApp::FinalizeContest() {
    ...
    if (mood < minMood) {
        ...
        logEvent("比赛结束后心态自动恢复：+" + std::to_string(recovery) +
            "，当前心态值：" + std::to_string(mood), "event");
    }
    ...
    if (isFinalDayContest) {
        if (currentContestName == "省选Day2") result.award = calculateAward(totalActualScore, "省选");
        else if (currentContestName == "NOI Day2") result.award = calculateAward(totalActualScore, "NOI");
        else if (currentContestName == "IOI Day2") result.award = calculateAward(totalActualScore, "IOI");
        else if (currentContestName == "CTT Day4") result.award = calculateAward(totalActualScore, "CTT");
        else if (currentContestName == "CTS Day2") result.award = calculateAward(totalActualScore, "CTS");
        else result.award = calculateAward(totalActualScore, currentContestName);
...
void GuiApp::HandleContestAction(int subProblemIdx, char action) {
    ...
    if (action == 'a') thinkSubProblem(problemIdx, subProblemIdx);
    else if (action == 'b') writeCodeSubProblem(problemIdx, subProblemIdx);
    else if (action == 'c') checkCodeSubProblem(problemIdx, subProblemIdx);
    else return;
...
void GuiApp::RenderTopBar() {
    ...
    ImGui::SameLine();
    ImGui::TextDisabled("基于 D:\\C++ Programs\\OI_Simulator 的核心数值与题库");
...
void GuiApp::RenderHome() {
    ...
    static const std::array<std::pair<const char*, const char*>, 4> difficultyOrder = {{
        {"easy", "简单：天赋点 30，初始决心 3000，分数线降低 20%"},
...
void GuiApp::RenderTraining() {
    ...
    if (training_.awaitingContinue) {
        ImGui::TextWrapped("当前训练事件已经处理完成。点击下面的按钮进入下一次训练事件，或者在最后一次事件后自动进入后续比赛。");
        ImGui::Spacing();
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow) {
    ...
    if (ImFont* font = LoadChineseFont(io, fontPath)) {
        io.FontDefault = font;
        App().SetFontWarning("当前字体：" + fontPath);
    } else {
