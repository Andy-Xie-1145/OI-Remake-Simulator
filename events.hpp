...
// ========== 商店状态（同一次进入时保持不变） ==========
inline std::set<std::string> purchasedItems;           // 已购买的商品
inline std::vector<int> currentShopItemIndices;        // 当前商店显示的商品索引
inline bool shopInitialized = false;                   // 商店是否已初始化
...
#ifdef _WIN32
    if (system("cls") != 0) {} // 忽略返回值
#else
    if (system("clear") != 0) {} // 忽略返回值
#endif
}

// ========== 初始化商店商品（只在第一次进入时调用） ==========
inline void initShopItems(const std::vector<EventOption>& options) {
    if (shopInitialized) return;  // 已初始化则跳过
    
    purchasedItems.clear();
    currentShopItemIndices.clear();
    
    // 找到所有非"放弃购买"的商品索引
    for (int i = 0; i < (int)options.size(); i++) {
        if (options[i].text != "放弃购买") {
            currentShopItemIndices.push_back(i);
        }
    }
    ...
    std::mt19937 g(rd());
    std::shuffle(currentShopItemIndices.begin(), currentShopItemIndices.end(), g);
    
    // 只保留前4个商品
    if (currentShopItemIndices.size() > 4) {
        currentShopItemIndices.resize(4);
    }
...
inline std::string runEvent(const std::string& eventName) {
    ...
    int leaveIndex = -1;  // "放弃购买"的索引
    
    // 商店特殊处理
    if (eventName == "决心商店") {
        // 初始化商店商品（如果还没初始化）
        initShopItems(options);
        ...
        // 添加未购买的商品到可用列表
        for (int idx : currentShopItemIndices) {
            if (purchasedItems.find(options[idx].text) == purchasedItems.end()) {
        ...
        // 添加"放弃购买"选项
        if (leaveIndex >= 0) {
            availableIndices.push_back(leaveIndex);
        }
    } else {
        // 非商店事件：按原逻辑处理
    ...
    for (int i = 0; i < (int)availableIndices.size(); i++) {
        ...
        // 商店商品显示当前价格
        if (eventName == "决心商店" && options[idx].text != "放弃购买") {
            int currentCost = currentShopPrices[options[idx].text];
            std::cout << " (花费" << currentCost << "点决心)";
        } else if (!options[idx].description.empty()) {
            std::cout << " (" << options[idx].description << ")";
        }
        std::cout << "\n";
    }
    
    std::cout << "当前决心: " << playerStats.determination << "\n";
    std::cout << "请选择: ";
    int choice;
    std::cin >> choice;
    ...
    if (eventName == "决心商店") {
        if (selected.text == "放弃购买") {
            logEvent("离开商店", "event");
            clearShopState();  // 清理商店状态
            return "";
        }
        
        int cost = currentShopPrices[selected.text];
        if (playerStats.determination < cost) {
            logEvent("决心不足！需要 " + std::to_string(cost) + " 决心，当前只有 " + std::to_string(playerStats.determination) + " 决心", "event");
            return "决心商店";  // 继续商店，商品列表保持不变
        }
        ...
        logEvent("下次购买" + selected.text + "需要 " + std::to_string(currentShopPrices[selected.text]) + " 决心", "event");
